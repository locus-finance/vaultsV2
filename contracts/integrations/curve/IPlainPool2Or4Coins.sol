...
interface IPlainPool2Or4Coins {
    ...
    function remove_liquidity(uint256 _amount, uint256[2] memory _min_amounts) external returns (uint256[] memory);
    ...
    /// @dev StableSwap.add_liquidity(_amounts: uint256[N_COINS], _min_mint_amount: uint256, _receiver: address = msg.sender) → uint256
    function add_liquidity(uint256[4] memory _amounts, uint256 _min_mint_amount, address _receiver) external returns (uint256);

    /// @dev StableSwap.remove_liquidity(_amount: uint256, _min_amounts: uint256[N_COINS]) → uint256[N_COINS]
    function remove_liquidity(uint256 _amount, uint256[4] memory _min_amounts) external returns (uint256[] memory);
